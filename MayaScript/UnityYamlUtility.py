...
def readUnityYamlData(filePath):
    ...
    objectHeaderの行をKey・内容をValueとしたcontentDictを返す
    """
    topHeader    = str()
    objectHeader = str()
    contentDict  = {}
    ...
    with open(filePath,"r") as rfile:
        for lineNumber,line in enumerate( rfile.readlines() ):
            ...
            if line.startswith("%YAML") or line.startswith("%TAG"):
                ...
                objectHeader = line
                contentDict[objectHeader] = str()
            else:
                contentDict[objectHeader] += line
                #contentDict += line

    return topHeader, contentDict


def writeUnityYamlData(topHeader, contentDict, filePath):
    """
    topHeader, contentDictの内容を filePath で指定したファイルに出力する
    """
    # 書き込みの処理
    with open(filePath,"w") as wfile:
        ...
        for objectHeader in contentDict.keys():
            wfile.write(objectHeader)
            # yaml.dump でMaya側から書き出したい内容を書く
            yaml.dump(contentDict[objectHeader], wfile)
...
def getComponentList(contentDict):
    """
    指定したcontentDict内を検索して、何のコンポーネント or オブジェクトのリストを取得
    """
    componentList = []
...
def getPropetyListInComponent(contentDict, classID):
    """
    ...
    for objectHeader, objectData in unityStreamContent.items():

        if hasTargetClass == True:
            break
